...
function Tracker(){
    ...
    if(!loaded){
        ...
        fetch(`${process.env.REACT_APP_APIBASEURL}/api/position?person_id=${id}&key=${localStorage.getItem('key')}`)
        .then(res=>{
            if(res.status==200){
                return res.json();
            }else{
        ...
        .then(
            (result)=>{
                ...
                let a = [];
                result.forEach(item=>{
                    a.push({coordinates:[parseInt(item.longitude),parseInt(item.latitude)]});
                    // setMarkers([...markers,{coordinates: [parseInt(item.latitude),parseInt(item.longitude)]}])
                    // console.log({coordinates:[parseInt(item.latitude),parseInt(item.longitude)]})
                });
                console.log(a);
                setMarkers(a);
            },(err)=>{
        ...
        fetch(`${process.env.REACT_APP_APIBASEURL}/api/person?person_id=${id}&key=${localStorage.getItem('key')}`)
            .then(res=>{
                if(res.status==200){
                    return res.json();
                }else{
                                ...
                                <tbody className="text-gray-600 text-sm font-light">
                                    {table.map((item,index)=>(
                                        <tr className="border-b border-gray-200 hover:bg-gray-100">
                                            ...
                                            <td className="py-3 px-6 text-center">{item.longitude}</td>
                                            <td className="py-3 px-6 text-center">{item.latitude}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
